          ...
          .map((year) => (
            <div className="group/list" key={year}>
              <div className="border-b-[1px] border-solid border-gray-200 pb-1 text-sm">
                <div className="group-hover/list:selection w-fit p-1">{year}</div>
              </div>
              <ul className="flex flex-col py-4">
                {postsByYear[year].map((post) => (
                  <li key={post.slug} className="group/item text-gray-800">
                    <a
                      className="flex w-full justify-between transition-opacity hover:!opacity-100 group-hover:opacity-40"
                      href={post.slug}
                    >
                      <div className="group-hover/item:selection p-1">{post.title}</div>
                      <div className="group-hover/item:selection p-1 text-sm">{post.date}</div>
                    </a>
                  </li>
                ))}
              </ul>
